...
2. **Consider all environmental factors** and their interactions
3. **Be realistic** - don't always predict perfect yields
4. **Vary predictions** based on different input conditions
...
- Consider that extreme temperatures, poor soil pH, excessive fertilizer, or low rainfall should reduce yields
- Good conditions should result in moderate to good yields (0.6-0.85)
- Poor conditions should result in low to moderate yields (0.2-0.6)
- Very poor conditions should result in low yields (0.1-0.4)`;
  }
...
- Consider economic viability of recommendations`;
  ...
  getFallbackPrediction(inputFeatures) {
    // Generate a simple fallback prediction based on environmental conditions
    let baseYield = 0.6; // Base 60% yield
    let baseRisk = 0.3; // Base 30% risk
    ...
    if (fertilizer < 20 || fertilizer > 150) {
      baseYield -= 0.15;
      baseRisk += 0.15;
    }
    ...
    return {
      ...
      risk_score,
      confidence: 0.6,
      reasoning: 'Fallback prediction based on environmental conditions',
      source: 'fallback'
    };
  }

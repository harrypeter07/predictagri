...
import { deviceTimezoneService } from './deviceTimezoneService.js'
...
class FarmerLocationService {
  ...
  async getLocationMetadata(lat, lon) {
    try {
      // Get timezone data from device instead of API
      let timezoneData = null
      
      try {
        // Use device timezone service instead of external API
        timezoneData = deviceTimezoneService.getDeviceTimezone()
        this.logger.info('device_timezone_used', { 
          timezone: timezoneData.timezone,
          gmtOffset: timezoneData.gmtOffset,
          source: timezoneData.source
        })
      } catch (e) {
        this.logger.warn('device_timezone_failed', { error: e.message })
  ...
  getFallbackCoordinates(address) {
    ...
    // Try to match by common village/town keywords - use generic state centers instead of hardcoded villages
    const villageKeywords = {
      'wari': { lat: 20.5937, lon: 78.9629, region: 'Central India' }, // Generic central India
    ...
    for (const [keyword, data] of Object.entries(villageKeywords)) {
      if (address.toLowerCase().includes(keyword)) {
        return {
          ...
          confidence: 0.6, // Lower confidence for generic village matches
          source: 'Fallback Data (Generic Village-based)',
          timestamp: new Date().toISOString()
        }
      }
    }

    // Default fallback coordinates (Center of India - more generic)
    return {
      lat: 20.5937,
      lon: 78.9629,
      displayName: `${address}, Central India`,
      address: { country: 'India', state: 'Central India' },
      confidence: 0.3, // Very low confidence for generic fallback
      source: 'Fallback Data (Generic Center)',
      timestamp: new Date().toISOString()
    }
  }
  ...
  getFallbackFarmerLocationData(farmerInput) {
    // Use provided coordinates or generic center of India
    const fallbackCoords = farmerInput.coordinates || { lat: 20.5937, lon: 78.9629 } // Center of India
    return {
      farmerId: farmerInput.farmerId || 'fallback_farmer_001',
      coordinates: fallbackCoords,
      address: { displayName: 'Central India', coordinates: fallbackCoords },

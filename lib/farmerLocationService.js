...
class FarmerLocationService {
  ...
  async getAddressFromCoordinates(lat, lon) {
    try {
      ...
      return this.getMockAddress(lat, lon)
    }
  }
  ...
  async getLocationMetadata(lat, lon) {
    try {
      // Get timezone data
      const timezoneUrl = `https://api.timezonedb.com/v2.1/get-time-zone?key=${process.env.TIMEZONE_API_KEY || 'demo'}&format=json&by=position&lat=${lat}&lng=${lon}`
      ...
      return this.getMockLocationMetadata(lat, lon)
    }
  }
  ...
  async getFarmerLocationData(farmerInput) {
    try {
      ...
      return {
        success: false,
        error: error.message,
        data: this.getMockFarmerLocationData(farmerInput)
      }
    }
  }
  ...
  getMockCoordinates(address) {
    ...
    // Default mock coordinates (Delhi)
    return {
      lat: 28.6139,
      lon: 77.2090,
  ...
  getMockLocationMetadata(lat, lon) {
    return {
      coordinates: { lat: parseFloat(lat), lon: parseFloat(lon) },
      timezone: { timezone: 'Asia/Kolkata', gmtOffset: 19800, dst: false },
      elevation: { elevation: 200, source: 'Mock Data' },
      timestamp: new Date().toISOString()
    }
  }

  getMockFarmerLocationData(farmerInput) {
    const mockCoords = { lat: 21.1458, lon: 79.0882 } // Nagpur
    return {
      farmerId: farmerInput.farmerId || 'mock_farmer_001',
      coordinates: mockCoords,
      address: { displayName: 'Nagpur, Maharashtra, India', coordinates: mockCoords },
      metadata: this.getMockLocationMetadata(mockCoords.lat, mockCoords.lon),
      agriculturalZone: this.classifyAgriculturalZone(mockCoords.lat, mockCoords.lon),
      soilClassification: this.classifySoilType(mockCoords.lat, mockCoords.lon),
      confidence: 0.8,
      timestamp: new Date().toISOString()
    }
  }
...
module.exports = FarmerLocationService

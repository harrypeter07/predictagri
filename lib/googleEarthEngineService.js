...
class GoogleEarthEngineService {
  ...
  async getComprehensiveSoilData(region, date = new Date()) {
    ...
    try {
      ...
      console.log(`🌍 [GEE] Fetching soil datasets in parallel...`)
      ...
      console.log(`🌍 [GEE] All soil datasets fetched successfully`)
      ...
      console.log(`🌍 [GEE] Comprehensive soil data fetch completed successfully for ${region.name}`)
      return result
    } catch (error) {
  ...
  async getSoilMoistureData(regionGeometry, date) {
    try {
      console.log(`🌍 [GEE] Fetching SMAP soil moisture data`)
      ...
      console.log(`🌍 [GEE] SMAP collection filtered and selected`)

      const smapImage = smapCollection.first()
      console.log(`🌍 [GEE] SMAP image retrieved:`, smapImage ? smapImage.toString() : 'null')
      
      if (!smapImage) {
        throw new Error('No soil moisture data available for the specified region and date')
      }
      
      console.log(`🌍 [GEE] Computing SMAP statistics`)
      ...
      console.log(`🌍 [GEE] SMAP soil moisture result:`, result)
  ...
  async getSoilTemperatureData(regionGeometry, date) {
    try {
      console.log(`🌍 [GEE] Fetching MODIS land surface temperature as soil temperature proxy`)
      ...
      console.log(`🌍 [GEE] MODIS LST collection filtered and selected`)

      const lstImage = lstCollection.first()
      console.log(`🌍 [GEE] MODIS LST image retrieved:`, lstImage ? lstImage.toString() : 'null')
      
      if (!lstImage) {
        throw new Error('No land surface temperature data available for the specified region and date')
      }
      
      console.log(`🌍 [GEE] Computing MODIS LST statistics`)
      ...
      console.log(`🌍 [GEE] MODIS LST result:`, { raw: result, celsius: tempCelsius })
  ...
  async getSoilOrganicCarbonData(regionGeometry, date) {
    try {
      console.log(`🌍 [GEE] Fetching soil organic carbon data for region geometry`)
      
      // Get SoilGrids250m data for organic carbon - this is an Image, not ImageCollection
      const soilGridsImage = ee.Image('projects/soilgrids-isric/clay_mean')
      console.log(`🌍 [GEE] SoilGrids clay image loaded:`, soilGridsImage.toString())
      
      // Apply filterBounds to the image
      const filteredImage = soilGridsImage.filterBounds(regionGeometry)
      console.log(`🌍 [GEE] Filtered image created:`, filteredImage.toString())

      const stats = filteredImage.reduceRegion({
        reducer: ee.Reducer.mean(),
        geometry: regionGeometry,
        scale: 250,
        maxPixels: 1e6
      ...
      console.log(`🌍 [GEE] Stats computed, extracting clay_mean value`)
      const result = await stats.get('clay_mean').getInfo()
      console.log(`🌍 [GEE] Soil organic carbon value:`, result)
  ...
  async getSoilPhData(regionGeometry, date) {
    try {
      console.log(`🌍 [GEE] Fetching soil pH data for region geometry`)
      ...
      console.log(`🌍 [GEE] SoilGrids pH image loaded:`, soilGridsImage.toString())
      
      // Apply filterBounds to the image
      const filteredImage = soilGridsImage.filterBounds(regionGeometry)
      console.log(`🌍 [GEE] Filtered pH image created:`, filteredImage.toString())
      ...
      console.log(`🌍 [GEE] pH stats computed, extracting phh2o_mean value`)
      const result = await stats.get('phh2o_mean').getInfo()
      console.log(`🌍 [GEE] Soil pH value:`, result)
  ...
  async getSoilTextureData(regionGeometry, date) {
    try {
      console.log(`🌍 [GEE] Fetching soil texture data for region geometry`)
      ...
      console.log(`🌍 [GEE] Soil texture images loaded:`, {
        clay: clayImage.toString(),
        silt: siltImage.toString(),
        sand: sandImage.toString()
      })

      // Apply filterBounds to each image
      const filteredClayImage = clayImage.filterBounds(regionGeometry)
      const filteredSiltImage = siltImage.filterBounds(regionGeometry)
      const filteredSandImage = sandImage.filterBounds(regionGeometry)
      
      console.log(`🌍 [GEE] Filtered texture images created`)

      const [clayStats, siltStats, sandStats] = await Promise.all([
        filteredClayImage.reduceRegion({
          reducer: ee.Reducer.mean(),
          geometry: regionGeometry,
          scale: 250,
          maxPixels: 1e6
        }),
        filteredSiltImage.reduceRegion({
          reducer: ee.Reducer.mean(),
          geometry: regionGeometry,
          scale: 250,
          maxPixels: 1e6
        }),
        filteredSandImage.reduceRegion({
          reducer: ee.Reducer.mean(),
          geometry: regionGeometry,
          scale: 250,
          maxPixels: 1e6
      ...
      console.log(`🌍 [GEE] Texture stats computed, extracting values`)
      ...
      console.log(`🌍 [GEE] Soil texture values:`, { clay, silt, sand })
      
      const texture = this.classifySoilTexture(clay, silt, sand)
      console.log(`🌍 [GEE] Classified soil texture:`, texture)
  ...
  async getLandUseData(region, date = new Date()) {
    try {
      ...
      console.log(`🌍 [GEE] Fetching ESA WorldCover data`)
      const worldCoverCollection = ee.ImageCollection('ESA/WorldCover/v100')
        .filterBounds(regionGeometry)
        .filterDate(date, new Date(date.getTime() + 365 * 24 * 60 * 60 * 1000))
      
      console.log(`🌍 [GEE] WorldCover collection filtered`)
      const worldCoverImage = worldCoverCollection.first()
      console.log(`🌍 [GEE] WorldCover image retrieved:`, worldCoverImage ? worldCoverImage.toString() : 'null')

      if (!worldCoverImage) {
        throw new Error('No land cover data available for the specified region and date')
      }

      console.log(`🌍 [GEE] Computing WorldCover frequency histogram`)
      ...
      console.log(`🌍 [GEE] WorldCover result:`, result)
  ...
  async getNDVIData(region, date = new Date()) {
    try {
      ...
      console.log(`🌍 [GEE] Fetching NDVI data for region: ${region.name}`)
      ...
      console.log(`🌍 [GEE] NDVI collection filtered and selected`)

      // Get the most recent image
      const ndviImage = ndviCollection.first()
      console.log(`🌍 [GEE] NDVI image retrieved:`, ndviImage ? ndviImage.toString() : 'null')
      
      if (!ndviImage) {
        throw new Error('No NDVI data available for the specified region and date')
      }
      
      // Calculate statistics
      console.log(`🌍 [GEE] Computing NDVI statistics`)
      ...
      console.log(`🌍 [GEE] NDVI result:`, result)
      this.incrementApiCall()

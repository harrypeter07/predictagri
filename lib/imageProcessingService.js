  ...
  async analyzeAgriculturalImage(imageBuffer, analysisType = 'comprehensive') {
    try {
      ...
      if (!imageInfo.isValid) {
        throw new Error(`Invalid image: ${imageInfo.error}`)
      }
      ...
      switch (analysisType) {
        case 'crop-health':
          analysisResult.results = await this.analyzeCropHealth(imageBuffer)
          break
        case 'disease-detection':
          analysisResult.results = await this.detectDiseases(imageBuffer)
          break
        case 'soil-analysis':
          analysisResult.results = await this.analyzeSoil(imageBuffer)
          break
        case 'weed-detection':
          analysisResult.results = await this.detectWeeds(imageBuffer)
          break
        case 'comprehensive':
        default:
          analysisResult.results = await this.comprehensiveAnalysis(imageBuffer)
          break
      }
      ...
      return {
        success: false,
        error: error.message,
        fallbackData: this.getMockAnalysisResult(analysisType)
      }
    }
  }
  ...
  async detectWeeds(imageBuffer) {
    try {
      ...
      return {
        ...
        detectedWeeds: weedTypes,
        affectedAreas: weedDetection.regions,

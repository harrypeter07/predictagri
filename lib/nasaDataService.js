...
class NASADataService {
  ...
  async getNaturalDisasters(limit = 10) {
    ...
    console.log(`🚀 [NASA] Fetching natural disasters with limit: ${limit}`)
    console.log(`🚀 [NASA] EONET URL: ${url.replace(this.apiKey, '***API_KEY***')}`)
    this.logger.info('eonet_request', { url })
    
    try {
      console.log(`🚀 [NASA] Making request to EONET API...`)
      const response = await fetchWithRetry(url) // Use fetchWithRetry
      
      if (response.ok) {
        console.log(`🚀 [NASA] EONET API response successful (${response.status})`)
        const data = await response.json()
        const eventCount = data.events?.length || 0
        console.log(`🚀 [NASA] Retrieved ${eventCount} natural disaster events`)
        ...
        console.error(`🚀 [NASA] EONET API failed with status ${response.status}:`, errorText)
        this.logger.warn('eonet_api_failed', { status: response.status, error: errorText })
        console.log(`🚀 [NASA] Falling back to fallback data`)
        return this.getFallbackNaturalDisasters() // Fallback to fallback data
      }
    } catch (error) {
      console.error(`🚀 [NASA] EONET API request failed:`, error)
      this.logger.error('eonet_fetch_error', { error: error.message })
      console.log(`🚀 [NASA] Falling back to fallback data due to network error`)
      return this.getFallbackNaturalDisasters() // Fallback to fallback data
    }
  }

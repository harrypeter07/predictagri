...
class NASADataService {
  ...
  async getNaturalDisasters(limit = 10) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.logger.info('eonet_success', { count: data.events?.length || 0 })
        return {
          success: true,
          data: data.events || [],
        ...
        return this.getMockNaturalDisasters() // Fallback to mock
      }
    } catch (error) {
  ...
  getMockNaturalDisasters() {
    this.logger.warn('eonet_fallback_mock', { message: 'Using mock data for natural disasters.' })
    return {
      success: false,
      data: [
        {
  ...
  getMockEarthImagery(lat, lon) {
    this.logger.warn('earth_imagery_fallback_mock', { message: 'Using mock data for Earth imagery.' })
    return {
      success: false,
      data: {
        lat: lat,
  ...
  getMockEPICImages() {
    this.logger.warn('epic_fallback_mock', { message: 'Using mock data for EPIC images.' })
    return {
      success: false,
      data: [
        {
  ...
  getMockAPOD() {
    this.logger.warn('apod_fallback_mock', { message: 'Using mock data for APOD.' })
    return {
      success: false,
      data: {
  ...
  getMockAgriculturalInsights() {
    this.logger.warn('agricultural_insights_fallback_mock', { message: 'Using mock data for agricultural insights.' })
    return {
      success: false,
      data: {
        timestamp: new Date().toISOString(),

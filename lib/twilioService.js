...
class TwilioService {
  ...
  async sendVoiceAlert(phoneNumber, message, language = 'en') {
    ...
    try {
      ...
      // Set language-specific voice with better parameters
      const voiceParams = this.getVoiceParams(language)
      
      // Use Gather with speech input to prevent key press interruptions
      const gather = twiml.gather({
        input: 'speech',
        actionOnEmptyResult: 'true',
        timeout: 0, // No timeout
        finishOnKey: '', // No key will finish
        speechTimeout: 'auto'
      })
      
      // Add a pause before speaking to ensure connection is stable
      gather.pause({ length: 1 })
      
      // Say the message with voice parameters
      gather.say({
        ...voiceParams
      }, message)
      
      // Add a pause after the message
      gather.pause({ length: 2 })
      
      // Add a gentle ending message
      const endMessage = language === 'hi' ? 'धन्यवाद।' : language === 'mr' ? 'धन्यवाद।' : 'Thank you.'
      gather.say(voiceParams, endMessage)
      
      // Add final pause before ending
      gather.pause({ length: 1 })
      
      // Explicitly hang up the call to ensure clean ending
      twiml.hangup()
      ...
      const result = await this.client.calls.create({
        ...
        from: this.phoneNumber,
        to: phoneNumber,
        // Add call parameters for better stability
        timeout: 30, // Call timeout in seconds
        record: false, // Don't record the call
        statusCallback: null // No status callback to avoid interference
      })
  ...
  async sendAgriculturalAlert(phoneNumber, alertData, language = 'en') {
    ...
    // Try SMS first, then voice as fallback
    let smsResult = await this.sendSMSAlert(phoneNumber, message, language)
    
    // If SMS fails due to limit, try voice only
    if (!smsResult.success && smsResult.error && smsResult.error.includes('exceeded')) {
      this.logger.warn('sms_limit_exceeded', { phoneNumber, error: smsResult.error })
      const voiceResult = await this.sendVoiceAlert(phoneNumber, message, language)
    ...
    // If SMS succeeds, also try voice for redundancy
    const voiceResult = await this.sendVoiceAlert(phoneNumber, message, language)

...
class WeatherService {
  ...
  async getCurrentWeather(lat, lon) {
    // Only try to fetch weather if we have a valid API key
    if (OPENWEATHER_API_KEY && OPENWEATHER_API_KEY !== 'demo_key') {
      ...
      console.log('OpenWeather API skipped - no valid API key provided')
    }
  ...
  async getWeatherForecast(lat, lon) {
    // Only try to fetch forecast if we have a valid API key
    if (OPENWEATHER_API_KEY && OPENWEATHER_API_KEY !== 'demo_key') {
      try {
        const response = await axios.get(
      ...
      console.log('OpenWeather Forecast API skipped - no valid API key provided')
    }

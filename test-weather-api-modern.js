// Modern Weather API Test Script (Node.js 18+ with built-in fetch)
// Run with: node test-weather-api-modern.js
...
// Test specific weather parameters
async function testWeatherParameters() {
  console.log('\n🔍 Testing Different Weather Parameters...\n');
  
  const location = testLocations[0]; // Use Mumbai for parameter testing
  
  const parameterTests = [
    {
      name: 'Basic Current Weather',
      url: `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&current=temperature_2m&timezone=auto`
    },
    {
      name: 'Extended Current Weather',
      url: `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code,pressure_msl&timezone=auto`
    },
    {
      name: 'Hourly Forecast',
      url: `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m&timezone=auto`
    },
    {
      name: 'Daily Forecast Extended',
      url: `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,weather_code&timezone=auto`
    }
  ];
  ...
  for (const test of parameterTests) {
    try {
      ...
      if (response.ok) {
        if (data.current) {
          console.log(`✅ Current: Temp: ${data.current.temperature_2m}°C, Humidity: ${data.current.relative_humidity_2m}%, Wind: ${data.current.wind_speed_10m} km/h`);
        }
        if (data.hourly) {
          console.log(`✅ Hourly: ${data.hourly.time?.length || 0} time slots available`);
        }
        if (data.daily) {
          console.log(`✅ Daily: ${data.daily.time?.length || 0} days forecast available`);
        }
      } else {
